...
router.post('/Enviar', function(req, res, next) {
  ...
  if (!iso || !ip || !puerto) {
    ...
    client.connect(puerto, ip, function() {
      //para TCPP hay que enviar el tamaño del mensaje en la cabecera del mensaje
      var length = iso.length;

      // en caso de hacer una conexion TCPP descomentar la linea de abajo y comentar la de TCP
    ...
    client.on('close', function() {
      ...
      //res.send({ error: '<div class="card-panel red darken-2" style="color: rgba(255, 255, 255, 0.9);"><span>Algo Salió Mal: no se pudo establecer una conexion al ip y puerto indicado.</span></div>' });
    });
  }
});
